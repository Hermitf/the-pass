import { Form, Input, Button, Typography, Divider, Tabs, Space, message, Radio, type RadioChangeEvent } from 'antd'
...
export default function LoginForm() {
  ...
  // const [loginRole, setLoginRole] = useState<'merchant' | 'user'>('merchant');
  // type TabPosition = '商家' | '用户';
  type LoginRole = '商家' | '用户'
  const [loginRole, setLoginRole] = useState<LoginRole>('商家')
  const navigate = useNavigate()

  const handleLoginRoleChange = (e: RadioChangeEvent) => {
      ...
      <div className='form-content simple-form'>
        ...
        >
          <Space.Compact style={{ width: '100%' }}>
            ...
            <Button style={{ height: '40px' }} type='primary' disabled={countdown > 0} onClick={sendSmsCode}>
      ...
      <div className='login-right'>
        <div className='login-form-container'>
          <div className='login-header'>
            <h2 className='login-title'>{loginRole}登录</h2>
            <p className='login-subtitle'>请选择您的登录方式</p>
          </div>
          <div className='login-role-switch'>
            <span className='login-role-label'>登录身份：</span>
            <Radio.Group
              value={loginRole}
              onChange={handleLoginRoleChange}
              className='login-role-radio-group'
              optionType='button'
              buttonStyle='solid'
            >
              <Radio.Button value='商家'>商家</Radio.Button>
              <Radio.Button value='用户'>用户</Radio.Button>
            </Radio.Group>
          </div>
          <Form className='login-form' onFinish={values => handleLogin(values, activeTab)}>
            <div className='tabs-container'>
              <Tabs activeKey={activeTab} onChange={setActiveTab} centered animated={{ tabPane: true }}>
                <TabPane tab='邮箱登录' key='email'>
                  <EmailLoginForm />
                </TabPane>
                <TabPane tab='手机密码' key='phone-password'>
                  <PhonePasswordLoginForm />
                </TabPane>
                <TabPane tab='短信登录' key='phone-sms'>
                  <PhoneSmsLoginForm />
                </TabPane>
